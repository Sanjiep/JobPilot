...
function MonthYearPicker({
  ...
  const YEARS = Array.from({ length: 80 }, (_, i) => new Date().getFullYear() - i + 10)
  ...
  return (
    <div ref={ref} style={{ position: 'relative', width: '100%' }}>
      <button
        type="button"
        disabled={disabled}
        onClick={() => { if (!disabled) { setOpen(o => !o); setShowYearDropdown(false) } }}
          ...
          >
            ...
            <div className="flex items-center justify-between px-4 py-3"
              ...
              <button
                ...
                style={{ color: '#9090b0', background: 'transparent', cursor: 'pointer' }}
              ...
              <div className="flex items-center gap-1">
                <span className="text-sm font-semibold px-2" style={{ color: '#ffffff' }}>
                  {selectedMonth !== null && selectedYear === viewYear ? MONTHS[selectedMonth] : '—'}
                </span>
                <button
                  type="button"
                  ...
                  style={{
                    color: '#ffffff',
                    background: showYearDropdown ? 'rgba(99,102,241,0.2)' : 'transparent',
                    border: showYearDropdown ? '1px solid rgba(99,102,241,0.4)' : '1px solid transparent',
                    cursor: 'pointer',
                  }}
                  onMouseEnter={e => { if (!showYearDropdown) e.currentTarget.style.background = 'rgba(255,255,255,0.08)' }}
            ...
            <AnimatePresence>
              {showYearDropdown && (
                <motion.div
                  ...
                  style={{ maxHeight: '200px', background: '#0e0e1a', borderBottom: '1px solid rgba(255,255,255,0.06)' }}
                >
                  {YEARS.map(year => {
                    ...
                    return (
                      <button
                        ...
                        onMouseEnter={e => { if (!isActive) e.currentTarget.style.background = 'rgba(99,102,241,0.12)' }}
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        cursor: 'pointer',
                        boxShadow: isSelected ? '0 2px 12px rgba(99,102,241,0.4)' : 'none',
                      }}
                      onMouseEnter={e => {
                        if (!isSelected) {
            ...
            {value && (
              <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', padding: '8px' }}>
                <button
                  ...
                  style={{ color: '#505070', background: 'transparent', cursor: 'pointer' }}
                  onMouseEnter={e => {
...
export default function Step4Experience({ onNext, onBack, data, dark, textPrimary, textMuted, border }: Props) {
  const saved = data?.experience
  const [items, setItems] = useState<Experience[]>(
    saved && saved.length > 0 ? saved : [empty()]
  )
  const [openId, setOpenId] = useState<string>(
    saved && saved.length > 0 ? saved[0].id : items[0].id
  )
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-12">
      ...
      >
        ...
        <h1 className="text-3xl font-bold mb-2" style={{ color: textPrimary }}>Work experience</h1>
        <p className="text-base" style={{ color: textMuted }}>Add your work history. Start with your most recent position.</p>
      </motion.div>
      ...
      <div className="space-y-4 mb-6">
        {items.map((exp, i) => (
          <motion.div
            ...
            style={{ border: `1px solid ${openId === exp.id ? '#6366f1' : border}` }}
          >
            {/* Accordion header */}
            <div
            ...
            >
              <div>
                ...
                {exp.company && (
                  <div className="text-xs mt-0.5" style={{ color: textMuted }}>{exp.company}</div>
                )}
              </div>
              <div className="flex items-center gap-2">
                ...
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none"
                  stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
                  style={{ color: textMuted, transform: openId === exp.id ? 'rotate(180deg)' : 'rotate(0)', transition: 'transform 0.2s' }}>
                  <polyline points="6 9 12 15 18 9"/>
                </svg>
              </div>
            </div>
            ...
            <AnimatePresence>
              {openId === exp.id && (
                ...
                >
                  <div className="grid grid-cols-2 gap-4 p-5" style={{ background: dark ? '#070711' : '#ffffff' }}>

                    {/* Job Title */}
                    <div>
                      <label className="block text-xs font-semibold uppercase tracking-wide mb-1.5" style={{ color: textMuted }}>Job Title *</label>
                      <input
                        type="text"
                        value={exp.title}
                        onChange={e => update(exp.id, 'title', toTitleCase(e.target.value))}
                    ...
                    {/* Company */}
                    <div>
                      <label className="block text-xs font-semibold uppercase tracking-wide mb-1.5" style={{ color: textMuted }}>Company *</label>
                      <input
                        type="text"
                        value={exp.company}
                        onChange={e => update(exp.id, 'company', toTitleCase(e.target.value))}
                    ...
                    {/* Location */}
                    <div>
                      <label className="block text-xs font-semibold uppercase tracking-wide mb-1.5" style={{ color: textMuted }}>Location</label>
                      <input
                        type="text"
                        value={exp.location}
                        onChange={e => update(exp.id, 'location', toTitleCase(e.target.value))}
                    ...
                    {/* Start Date */}
                    <div>
                      <label className="block text-xs font-semibold uppercase tracking-wide mb-1.5" style={{ color: textMuted }}>Start Date</label>
                      <MonthYearPicker
                        value={exp.startDate}
                    ...
                    {/* End Date */}
                    <div>
                      <label className="block text-xs font-semibold uppercase tracking-wide mb-1.5" style={{ color: textMuted }}>End Date</label>
                      <MonthYearPicker
                        value={exp.endDate}
                    ...
                    <div className="col-span-2 flex items-center gap-3">
                      ...
                      />
                      <label htmlFor={`current-${exp.id}`} className="text-sm cursor-pointer" style={{ color: textMuted }}>
                    ...
                    <div className="col-span-2">
                      <label className="block text-xs font-semibold uppercase tracking-wide mb-1.5" style={{ color: textMuted }}>Description</label>
                      <textarea
                        rows={4}
      ...
      <div className="flex gap-3">
        ...
        <button
          onClick={() => onNext({ experience: items })}
