...
export default function OnboardingPage() {
  ...
  useEffect(() => {
    try {
      ...
      // Only restore if saved data belongs to current user
      if (savedData._userId && savedData._userId !== user?.id) {
        // Different user — clear and start fresh
  ...
  const goNext = (stepData?: Record<string, any>) => {
    ...
    const dataWithUser = { ...newData, _userId: user?.id }; // stamp userId
    localStorage.setItem("onboarding_data", JSON.stringify(dataWithUser)); // ← save dataWithUser not newData
