...
// Step Item
function StepItem({ step, isCompleted, isActive, isClickable, dark, textPrimary, textMuted, border, onClick }: {
  ...
  return (
    <div
      ...
      style={{
        background: isActive
          ? 'rgba(99,102,241,0.1)'
          : hovered
          ? dark ? 'rgba(255,255,255,0.05)' : '#f0f0ff'
          : 'transparent',
    ...
    >
      <div
        ...
        style={{
          background: isCompleted ? '#6366f1' : isActive ? 'rgba(99,102,241,0.2)' : border,
      ...
      <span className="text-sm flex-1 transition-all" style={{
      ...
      {isClickable && (
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none"
          ...
          style={{ color: textMuted, flexShrink: 0, opacity: hovered ? 1 : 0, transition: 'opacity 0.15s' }}>
...
// Theme Toggle
function ThemeToggle({ dark, toggle, border, textMuted, textPrimary }: {
  dark: boolean; toggle: () => void; border: string; textMuted: string; textPrimary: string
}) {
  return (
    <button
      onClick={toggle}
...
// User Menu
function UserMenu({ dark, textPrimary, textMuted, border }: {
  dark: boolean; textPrimary: string; textMuted: string; border: string
}) {
          ...
          >
            <div className="px-4 py-3" style={{ borderBottom: `1px solid ${border}` }}>
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <div className="text-sm font-semibold truncate" style={{ color: textPrimary }}>{displayName}</div>
                  <div className="text-xs truncate mt-0.5" style={{ color: textMuted }}>{user?.email ?? '—'}</div>
                </div>
              </div>
            </div>
            ...
            {menuItems.map(item => (
              <button key={item.label}
                ...
                style={{ background: 'transparent', color: textPrimary, cursor: 'pointer' }}
                onMouseEnter={e => e.currentTarget.style.background = dark ? 'rgba(255,255,255,0.05)' : '#f5f5ff'}
            ...
            <div style={{ borderTop: `1px solid ${border}` }}>
              <button
                ...
                className="w-full flex items-center gap-3 px-4 py-2.5 text-sm transition-all"
                style={{ background: 'transparent', color: '#f87171', cursor: 'pointer' }}
...
export default function OnboardingPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const savedHighest = parseInt(localStorage.getItem('onboarding_highest') || '1')
  ...
  const renderStep = () => {
    switch (currentStep) {
      case 1:  return <Step1Resume {...stepProps} />
      case 2:  return <Step2Personal {...stepProps} />
      case 3:  return <Step3Academic {...stepProps} />
      case 4:  return <Step4Experience {...stepProps} />
      case 5:  return <Step5Skills {...stepProps} />
      case 6:  return <Step6CoverLetter {...stepProps} />
      case 7:  return <Step7Situation {...stepProps} />
      case 8:  return <Step8WorkPrefs {...stepProps} />
      case 9:  return <Step9Countries {...stepProps} />
      case 10: return <Step10JobPrefs {...stepProps} />
      case 11: return <Step11Frequency {...stepProps} />
      case 12: return <Step12Email {...stepProps} />
      case 13: return <Step13Automation {...stepProps} />
      default: return null
    }
  }
        ...
        <div className="space-y-1 flex-1 overflow-y-auto">
          {STEPS.map(step => (
            <StepItem
              ...
              isClickable={step.number < highestStep && step.number !== currentStep}
              dark={dark}
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        >
          ...
          <div className="flex items-center gap-2 ml-auto">
            <ThemeToggle dark={dark} toggle={toggleTheme} border={border} textMuted={textMuted} textPrimary={textPrimary} />
            <UserMenu dark={dark} textPrimary={textPrimary} textMuted={textMuted} border={border} />
          </div>
        </div>

        {/* Mobile progress bar */}
        <div className="lg:hidden h-1 w-full" style={{ background: border }}>
          <motion.div className="h-full" style={{ background: '#6366f1' }}
